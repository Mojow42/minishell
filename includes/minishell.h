...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/11/19 02:47:01 by vpoelman          #+#    #+#             */
/*   Updated: 2025/11/19 02:47:04 by vpoelman         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void							add_token_to_list(t_token **head,
									t_token **current, char *content, int type);
...
void							handle_input_redir(char *input, int *i,
									t_token **head, t_token **current);
void							handle_output_redir(char *input, int *i,
									t_token **head, t_token **current);
t_redir							*add_redir_to_list(t_redir *redirs,
									t_redir_type type, char *content);
char							**add_arg_to_array(char **args,
									const char *new_arg);
t_cmd							*create_new_command(void);
t_token							*parse_command_tokens(t_cmd *cmd,
									t_token *tokens);
t_token							*handle_redir_token(t_cmd *cmd,
									t_token *tokens);
int								is_redir_token(int token_type);
int								execute_cmd(t_cmd *cmd, t_shell *shell);
int								execute_commands(t_cmd *cmds, t_shell *shell);
int								execute_pipeline_loop(t_cmd *cmds,
									t_shell *shell);
int								execute_single_command(t_cmd *cmds,
									t_shell *shell);
int								get_process_exit_code(int status);
int								handle_enoexec_error(char *path, char **args,
									char **env);
...
void							handle_dollar_expansion(char *clean_word,
									t_expand_ctx *ctx, t_shell *shell);
...
char							*ft_format_env(const char *key,
									const char *value);
...
int								read_heredoc_loop(int fd, char *delim,
									int quoted, t_shell *shell);
